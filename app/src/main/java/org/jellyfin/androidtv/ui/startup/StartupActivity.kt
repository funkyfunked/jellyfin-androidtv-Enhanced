...
class StartupActivity : FragmentActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		// Ensure basic permissions - skip for enhanced version on Fire TV devices
		if (applicationContext.packageName.contains(".enhanced") && android.os.Build.MODEL.contains("AFT")) {
			// For Fire TV devices with enhanced package, skip permission check
			Timber.i("Skipping permission check for enhanced version on Fire TV")
			onPermissionsGranted()
		} else {
			// Regular permission check for other cases
